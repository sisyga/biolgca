...
uptake 0.05 per cell, saturating at 0.1 in one route) on a periodic square lattice. The cells divide and die every step, so the matrix of a steady field
...
# route: (solver, backend, cell term), or None for the go-or-grow step
ROUTES = {
    "go_or_grow": None,
    "steady amg": ("steady", "amg", UPTAKE),
    "steady cg": ("steady", "cg", UPTAKE),
    "steady direct": ("steady", "direct", UPTAKE),
    "steady amg saturating": ("steady", "amg", SATURATING),
    "implicit cg": ("implicit", "cg", UPTAKE),
}
    ...
    if solver is None:
        ...
        operators = [{"name": "birth_death", "parameters": {"birth_rate": 0.05, "death_rate": 0.05}},
                     PDESpec(field="u", diffusion=1.0, decay=1e-3, production=1e-3, cells=[term], solver=name,
                             solver_options={"backend": backend})]
...
def main():
    ...
    if "steady direct" in results:
        ...
        for route in routes:
            if route.startswith("steady") and route != "steady direct":
