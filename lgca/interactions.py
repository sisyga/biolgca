...
def wetting(lgca):
    ...
    # nbs /= lgca.n_crit
    death = npr.random(lgca.nodes[lgca.spheroid].shape) < lgca.r_b / (rho0 + 1)
    ...
    pressure = np.clip(lgca.cell_density - rho0, a_min=0, a_max=None)
    # g_pressure = -lgca.gradient(pressure)
    pressure_weight = np.clip(lgca.cell_density[..., None] - 1 - rho0, a_min=0, a_max=None) - lgca.channel_weight(
        pressure)

    resting = lgca.nodes[..., lgca.velocitychannels:].sum(-1)
    resting = lgca.nb_sum(resting)
    resting *= (1 - resting / rho0 / lgca.velocitychannels / 2) * np.heaviside(
        1 - nbs / rho0 / lgca.velocitychannels / 2, 0) / lgca.velocitychannels / rho0 * 2
    ...
    for coord in zip(*coords):
        ...
        weights = np.exp(  # -lgca.beta * nbs[coord] * np.linalg.norm(j - j_nb[:, None], axis=0) / lgca.n_crit
            ...
            #+ lgca.beta * resting[coord] * restc * (1 - restc / rho0 / 2) * 2
