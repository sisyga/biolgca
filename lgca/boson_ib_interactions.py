    ...
    for coord in zip(*coords):
        ...
        for channel in node:
            for cell in channel:
                ...
                if ch_counter == 2:
                    if(npr.random()<=(tanh_switch(rho=rho, kappa=lgca.props['kappa'][cell], theta=lgca.props['theta'][cell])**(1/1.0))):#increasing resting cells probability to rest
                        restcells.append(cell)
                    else:
                        velcells.append(cell)
                    if(npr.random() <= lgca.r_b*(1-rho)):
                    ...
                    if(npr.random()<=(tanh_switch(rho=rho, kappa=lgca.props['kappa'][cell], theta=lgca.props['theta'][cell])**(1.0))): #reducing moving cells probability to rest
                        restcells.append(cell)
                    else:
                        velcells.append(cell)
            ch_counter +=1
