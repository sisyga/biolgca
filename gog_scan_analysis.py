...
fig, axes = plt.subplots((len(r_ds)-1)//stepsize, (len(thetas)-2)//stepsize, figsize=(10, 10), sharex=True, sharey=False)
lgca = get_lgca(**constparams)
# iterate over the grid
# print(r_ds.shape, thetas.shape, len(r_ds)//stepsize, len(thetas)//stepsize)
# for ax, index in zip(axes.flat, product(np.arange(1, len(r_ds), stepsize), np.arange(1, len(thetas)-1, stepsize))):
...
#     plt.sca(ax)
#     plt.hist(d['kappa'][d['nodes_t'].sum()], density=True, log=True)  # not really nodes_t, naming error! it's just the nodes of the last time step
