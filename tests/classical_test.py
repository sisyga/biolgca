...
class Test_LGCA_classical(T_LGCA_Common):
    ...
    * boundary conditions
    ...
    @pytest.fixture
    def out_1d_rbound(self, nodes_1d_rbound):
        # fixtures for 1D boundary conditions
    ...
    @pytest.fixture
    def out_1d_lbound(self, nodes_1d_lbound):
        # fixtures for 1D boundary conditions
        # common part of expected output right
    ...
    @pytest.fixture
    def out_sq_rbound(self, nodes_sq_rbound):
        # fixtures for 2D square boundary conditions
        # common part of expected output right
    ...
    @pytest.fixture
    def out_sq_lbound(self, nodes_sq_lbound):
        # fixtures for 2D square boundary conditions
    ...
    @pytest.fixture
    def out_sq_tbound(self, nodes_sq_tbound):
        # fixtures for 2D square boundary conditions
    ...
    @pytest.fixture
    def out_sq_bbound(self, nodes_sq_bbound):
        # fixtures for 2D square boundary conditions
    ...
    @pytest.fixture
    def out_hex_rbound(self, nodes_hex_rbound):
        # fixtures for 2D hex boundary conditions
        # common part of expected output right
    ...
    @pytest.fixture
    def out_hex_trbound(self, nodes_hex_trbound):
        # fixtures for 2D hex boundary conditions
    ...
    @pytest.fixture
    def out_hex_tlbound(self, nodes_hex_tlbound):
        # fixtures for 2D hex boundary conditions
    ...
    @pytest.fixture
    def out_hex_lbound(self, nodes_hex_lbound):
        # fixtures for 2D hex boundary conditions
        # common part of expected output left
    ...
    @pytest.fixture
    def out_hex_blbound(self, nodes_hex_blbound):
        # fixtures for 2D hex boundary conditions
    ...
    @pytest.fixture
    def out_hex_brbound(self, nodes_hex_brbound):
        # fixtures for 2D hex boundary conditions
    ...
    def t_propagation_template(self, geom, nodes, expected_output, bc='pbc'):
        ...
        lgca = get_lgca(geometry=geom, nodes=nodes, bc=bc, interaction='only_propagation')
    ...
    def test_pbc_square(self, nodes_sq_rbound, out_sq_rbound, nodes_sq_lbound, out_sq_lbound,
        ...
        # left boundary
        out_sq_lbound[2, :, 2] = 1  # particles that cross boundary
        ...
        # bottom boundary
        out_sq_bbound[:, 3, 3] = 1  # particles that cross boundary
    ...
    def test_rbc_square(self, nodes_sq_rbound, out_sq_rbound, nodes_sq_lbound, out_sq_lbound,
        ...
        # right boundary
        out_sq_rbound[2, :, 2] = 1  # particles that cross boundary
        ...
        # top boundary
        out_sq_tbound[:, 3, 3] = 1  # particles that cross boundary
    ...
    @pytest.mark.parametrize("geom,nodes", [
        ('lin', gen.nodes_ve_1d),
        ('square', gen.nodes_ve_square),
        ('hex', gen.nodes_ve_hex)
    ])
    def test_volume_exclusion(self, geom, nodes):
        # volume exclusion principle: test compliance in all interactions
        ref_lgca = get_lgca(geometry=geom)
        for interaction in ref_lgca.interactions:
            # test all boundary conditions in case of abuse of border nodes
            lgca = get_lgca(geometry=geom, nodes=nodes, interaction=interaction, bc='pbc')
            ...
            lgca = get_lgca(geometry=geom, nodes=nodes, interaction=interaction, bc='rbc')
            lgca.timeevo(timesteps=100, recorddens=False, record=True, showprogress=False)
            assert np.max(lgca.nodes.astype(int)) <= 1, "Volume exclusion principle is not respected"
            lgca = get_lgca(geometry=geom, nodes=nodes, interaction=interaction, bc='abc')
            lgca.timeevo(timesteps=100, recorddens=False, record=True, showprogress=False)
            assert np.max(lgca.nodes.astype(int)) <= 1, "Volume exclusion principle is not respected"