...
def out_hex_tlbound(nodes_hex_tlbound):
    ...
    expected_output[0, 1::2, 2] = np.array(
        [1, 3, 5]
    )  # particles that do not cross boundary
...
def out_hex_brbound(nodes_hex_brbound):
    ...
    expected_output[-1, 0::2, 5] = np.array(
        [2, 4, 6]
    )  # particles that do not cross boundary
...
class Test_LGCA_NoVE(T_LGCA_Common):
    ...
    * boundary conditions (uses tests from classical LGCA)
    ...
    def test_recording(self, geom, dims):
        ...
        assert not hasattr(
            lgca_1, "nodes_t"
        ), "Records node configuration when it should not"
        assert not hasattr(lgca_1, "n_t"), "Records particle number when it should not"
        assert not hasattr(lgca_1, "restcells_t") and not hasattr(
            lgca_1, "velcells_t"
        ), "Records density per channel type when it should not"
        assert (
            not hasattr(lgca_1, "ent_t")
            and not hasattr(lgca_1, "normEnt_t")
            and not hasattr(lgca_1, "polAlParam_t")
            and not hasattr(lgca_1, "meanAlign_t")
        ), "Records order parameters when it should not"
        del lgca_1
        ...
        lgca_5.timeevo(
            timesteps=2, recordpertype=True, recorddens=False, showprogress=False
        )
        assert hasattr(lgca_5, "restcells_t") and hasattr(
            lgca_5, "velcells_t"
        ), "Does not record density per channel type"
        del lgca_5

        lgca_6.timeevo(
            timesteps=2, recordorderparams=True, recorddens=False, showprogress=False
        )
        assert (
            hasattr(lgca_6, "ent_t")
            and hasattr(lgca_6, "normEnt_t")
            and hasattr(lgca_6, "polAlParam_t")
            and hasattr(lgca_6, "meanAlign_t")
        ), "Does not record order parameters"
    ...
    def test_propagation(self):
        ...
        expected_output = np.zeros(
            (self.xdim_square, self.ydim_square, self.b_square + 1)
        )
    ...
    def test_getlgca_capacity(self, geom, nodes, b):
        ...
        density = 0.5
        lgca = get_lgca(
            geometry=geom, ve=False, nodes=nodes, interaction="only_propagation"
        )
        ...
        ), "Capacity keyword cannot overwrite format of nodes"
        lgca = get_lgca(
            geometry=geom,
            ve=False,
            density=density,
    ...
    def test_pbc_1d(
        self, nodes_1d_rbound, out_1d_rbound, nodes_1d_lbound, out_1d_lbound
    ):
    ...
    def test_pbc_hex(
        ...
        # top right boundary
        out_hex_trbound[0, 1::2, 1] = np.array(
            [1, 3, 5]
        )  # particles that cross boundary
        ...
        out_hex_blbound[-1, 0::2, 4] = np.array(
            [2, 4, 6]
        )  # particles that cross boundary
        ...
        out_hex_brbound[0, 1::2, 5] = np.array(
            [3, 5, 1]
        )  # particles that cross boundary
    ...
    def test_rbc_hex(
        ...
        # top right boundary
        out_hex_trbound[-1, 0::2, 4] = np.array(
            [1, 3, 5]
        )  # particles that cross boundary
        ...
        # top left boundary
        out_hex_tlbound[0, 1::2, 5] = np.array(
            [2, 4, 6]
        )  # particles that cross boundary
        ...
        # bottom left boundary
        out_hex_blbound[0, 1::2, 1] = np.array(
            [2, 4, 6]
        )  # particles that cross boundary
        ...
        # bottom right boundary
        out_hex_brbound[-1, 0::2, 2] = np.array(
            [1, 3, 5]
        )  # particles that cross boundary
